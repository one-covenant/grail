...
class SGLangServerBackend(TextGenBackend):
    ...
    def _cap_max_tokens(self, prompt_len: int, max_new_tokens: int) -> int:
        """Decide whether ``prompt_len + max_new_tokens`` fits the model context.

        Returns ``max_new_tokens`` unchanged if it fits, ``0`` if it does not.

        We deliberately do NOT return a partially clamped value: the
        validator's ``TerminationValidator`` requires
        ``completion_length == min(metadata.max_tokens, protocol_cap)`` or
        EOS, so a completion that stops at a clamped intermediate length
        (e.g. 1288 instead of 2048) hard-fails ``termination_valid``.
        Returning 0 makes the caller skip the prompt entirely; the
        per-group ``CHALLENGE_K`` gate in
        ``cli/mine.py::_pipelined_generation_loop`` then drops the whole
        group, which is the right "loop iteration skip" crash domain.
        """
        if self._max_model_len is None:
            return int(max_new_tokens)
        room = int(self._max_model_len) - int(prompt_len)
        if room <= 0 or int(max_new_tokens) > room:
            return 0
        return int(max_new_tokens)
