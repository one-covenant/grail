...
class TestBurnMechanism:
    ...
    def test_underproduction_increases_burn_when_capped(self) -> None:
        ...
        hotkeys = ["burn", "m1"]
        uids = [0, 1]
        # 10% of per-window cap with rolling_windows=1 -> total_cap_relative ~ 0.1
        ten_pct = UNIQUE_ROLLOUTS_CAP // 10
        w = _compute(hotkeys, uids, {"m1": ten_pct}, {"m1": 1}, cap_enabled=True)
