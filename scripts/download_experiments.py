#!/usr/bin/env python3
"""Download experiments from R2 with parallel downloads.

Downloads:
- All delta files
- Checkpoints >= 350 only
- All log files
"""
...
EXPERIMENTS = [
    ("experiments/llama3.2-3b-iter1/", "experiments/llama3.2-3b-iter1/", "Llama 3.2 3B iter1"),
    ("qwen2.5-7b-grpo-math-lr3e-06/", "experiments/qwen2.5-7b-grpo-math-lr3e-06/", "Qwen 7B GRPO"),
    ("qwen2.5-1.5b-sft-math-lr2e-05/", "experiments/qwen2.5-1.5b-sft-math-lr2e-05/", "Qwen 1.5B SFT lr2e-05"),
    ("qwen2.5-1.5b-sft-math-lr3e-06/", "experiments/qwen2.5-1.5b-sft-math-lr3e-06/", "Qwen 1.5B SFT lr3e-06"),
]
...
# Thread-local storage for S3 clients (one per thread)
...
def get_s3_client():
    ...
    if not hasattr(thread_local, "s3"):
        ...
        thread_local.s3 = session.create_client(
            "s3",
...
def create_s3_client():
    """Create a new S3 client (for listing)."""
    session = get_session()
...
def should_download(key):
    ...
    if checkpoint_match:
        ...
        return checkpoint_num >= MIN_CHECKPOINT
    return False
...
def download_file(task: DownloadTask) -> tuple[bool, int]:
    """Download a single file using thread-local S3 client."""
    try:
...
class ProgressTracker:
    ...
    def _print_progress(self):
        ...
        if self.downloaded_bytes > 0 and elapsed > 0:
            ...
            eta_sec = bytes_remaining / (self.downloaded_bytes / elapsed)
        ...
        print(
            f"\rProgress: {self.completed_files}/{self.total_files} ({pct}%) | "
            f"Downloaded: {gb:.1f}GB | Speed: {speed:.1f}MB/s | "
            f"Errors: {self.errors} | {eta_str}    ",
            end="",
            flush=True,
        )
...
def download_experiment(r2_prefix, local_prefix, description):
    """Download an experiment from R2 with parallel downloads."""
    print(f"\n{'='*70}")
    ...
    # Filter and create download tasks
    tasks = []
    for obj in objects:
        key = obj["Key"]
...
def main():
    print("="*70)
    print("R2 Experiment Downloader (Parallel)")
    print(f"  Workers: {NUM_WORKERS}")
