...
assert (FULL_MODEL_OUTPUT_DIR / "model.safetensors").exists() or \
       (FULL_MODEL_OUTPUT_DIR / "pytorch_model.bin").exists(), "Model weights not found"
...
if True: 
    model.save_pretrained_merged(str(MERGED_32_DIR), tokenizer, save_method = "merged_32bit",)

# Also save 16bit version for deployment
if True: 
    model.save_pretrained_merged(str(MERGED_16_DIR), tokenizer, save_method = "merged_16bit",)
...
if False: 
    model.save_pretrained_merged(str(MERGED_4_DIR), tokenizer, save_method = "merged_4bit",)
